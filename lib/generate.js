...
async function readFileContent(filePath, options = {}) {
  try {
    ...
    if (analysis && (options.fileSummaries || options.includeRelationships || 
                     options.includeDependencies || options.includeFrameworkContext)) {
      ...
      // Framework context
      if (options.includeFrameworkContext && analysis.frameworks.length > 0) {
        contextParts.push(`Framework: ${analysis.frameworks.join(', ')}`);
        contextParts.push(`Language: ${analysis.language}`);
      }
...
export async function generateText(filePaths, options = {}) {
  ...
  if (options.projectAnalysis && (options.includeFrameworkContext || options.fileSummaries)) {
    ...
    if (stats.detectedFrameworks.length > 0) {
      summary += `Detected Frameworks: ${stats.detectedFrameworks.join(', ')}\n`;
    }
