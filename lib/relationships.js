...
/**
 * Detect frameworks used in a file
 */
 ...
 */
function generateFileSummary(content, filePath, language, imports, exports, frameworks) {
  ...
  const patterns = {
    ...
    isComponent: /component|\.component\.|\.jsx$|\.vue$/i.test(fileName),
  ...
  if (patterns.isTest) {
    ...
    summary = `Configuration file for ${frameworks.join(', ') || 'project'} setup`;
  } else if (frameworks.includes('react') && (patterns.isComponent || /\.jsx$/.test(fileName) || /export.*=.*\(\).*=>/.test(content))) {
    summary = `React component - ${fileName.replace(/\.[^.]+$/, '')}`;
  } else if (frameworks.includes('vue') && patterns.isComponent) {
    summary = `Vue component - ${fileName.replace(/\.[^.]+$/, '')}`;
  } else if (frameworks.includes('flask') || frameworks.includes('django')) {
    summary = `${frameworks.join(', ')} application module`;
  } else if (patterns.isService) {
    summary = `Service module for ${frameworks.join(', ') || 'application'} logic`;
  } else if (patterns.isRoute) {
    summary = `Route handler for ${frameworks.join(', ') || 'API'} endpoints`;
  } else if (patterns.isMiddleware) {
    summary = `Middleware for ${frameworks.join(', ') || 'request'} processing`;
...
export async function analyzeFileRelationships(filePath, content) {
  try {
    ...
    const frameworks = detectFrameworks(content, filePath);
    const summary = generateFileSummary(content, filePath, language, imports, exports, frameworks);
    ...
    return {
      ...
      imports: [],
      exports: [],
      stats: { importCount: 0, exportCount: 0, frameworkCount: 0 },
      error: error.message
    };
  }
}
...
export async function analyzeProjectRelationships(files) {
  ...
  return {
    ...
    stats: {
      ...
      detectedFrameworks: [...new Set(Array.from(relationships.values()).flatMap(r => r.frameworks))]
    }
  };
}
...
export function groupFilesByFunction(relationships, dependencyGraph) {
  ...
  const ungrouped = new Set(relationships.keys());
  
  // Group by framework/technology
  for (const [filePath, analysis] of relationships) {
    if (analysis.frameworks.length > 0) {
      for (const framework of analysis.frameworks) {
        if (!groups.has(framework)) {
          groups.set(framework, new Set());
        }
        groups.get(framework).add(filePath);
        ungrouped.delete(filePath);
      }
    }
  }
  
  // Group by file patterns and directory structure
  const remainingFiles = Array.from(ungrouped);
  ...
  for (const filePath of remainingFiles) {
    ...
    if (!pathGroups.has(category)) {
      pathGroups.set(category, new Set());
    }
    pathGroups.get(category).add(filePath);
  }
  
  // Merge path-based groups into main groups
  for (const [category, files] of pathGroups) {
    groups.set(category, files);
  }
